...
function PassMissmatch() {
    ...
    const handleEmailSend = async(e) =>{
        ...
        try {
            setIsLoading(true)
            const response = await axios.post('https://anonimous-mailserver.vercel.app/sendpassmissmatch',{email:receiverEmail});
            //toast.success(`Save ${response.data.receiver} successfully`);
            setIsLoading(false);
            Swal.fire({
                icon:'success',
                title:'Password Missmatch Email sent successfully',
            })
    ...
    return (
        <MainLayout>
            <div className="max-w-lg bg-white shadow-lg mx-auto p-7 rounded mt-6">
                <h2 className="font-semibold text-xl mb-4 text-center block">Send PasswordMissmatch</h2>
